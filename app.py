...
app.config['SECRET_KEY'] = 'dsfef778788trhj7tykj87s5f4esf'
...
class AddProductForm(FlaskForm):
    ...
    category = SelectField('Категория', choices=[
        ...
        ('Телевизоры и ТВ аксессуары', 'Телевизоры и ТВ аксессуары'),
        ('Компьютерная техника', 'Компьютерная техника'),
        ('Смартфоны и гаджеты', 'Смартфоны и гаджеты'),
        ('Умный дом', 'Умный дом')
...
def admin_orders():
    ...
    for order in orders:
        try:
            ...
            flash('Ошибка при разборе информации о товарах для заказа №{}'.format(order.id), 'error')
            continue
...
def remove_from_cart(product_id):
    ...
    flash('success', 'Товар успешно удален с корзины')
